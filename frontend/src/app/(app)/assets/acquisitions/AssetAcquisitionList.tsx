...
export default function AssetAcquisitionList({ dict, commonDict, attachmentsDict }: {
  ...
  function openCreate() { setEditing(null); setDraft(emptyDraft()); setError(null); }
  ...
  async function save(e: FormEvent) {
    ...
    try {
      ...
      setEditing(saved); load(page?.page ?? 0); setToast(commonDict.actionSuccess);
  ...
  return <>
    ...
    <div className="panel no-print">
      <div className="panel-head"><form className="filter-row" onSubmit={search}><input value={q} onChange={(e) => setQ(e.target.value)} placeholder={dict.searchPlaceholder} /><select value={assetFilter} onChange={(e) => { const next = e.target.value; setAssetFilter(next); load(0, size, appliedQuery, next); }}><option value="">{dict.allAssets}</option>{assets.map((a) => <option key={a.id} value={a.id}>{a.assetNumber} — {a.nameAr}</option>)}</select><input type="date" value={dateFrom} max={dateTo || undefined} onChange={(e) => { const next = e.target.value; setDateFrom(next); load(0, size, appliedQuery, assetFilter, next, dateTo); }} /><input type="date" value={dateTo} min={dateFrom || undefined} onChange={(e) => { const next = e.target.value; setDateTo(next); load(0, size, appliedQuery, assetFilter, dateFrom, next); }} /></form>{canManage && <button className="btn btn-primary btn-sm" type="button" onClick={openCreate}>{dict.addNew}</button>}</div>
    ...
    {editing !== undefined && <div className="overlay no-print" role="dialog" aria-modal="true"><div className="modal wide"><div className="modal-head"><h3>{editing ? dict.editTitle : dict.addNew}</h3><button className="modal-close" type="button" onClick={() => setEditing(undefined)}>×</button></div><div className="modal-body"><form id="acquisition-form" onSubmit={save} className="form-grid"><div className="field"><label>{dict.documentNumber}</label><input required value={draft.documentNumber} onChange={(e) => setDraft({ ...draft, documentNumber: e.target.value })} /></div><div className="field"><label>{dict.documentDate}</label><input type="date" required value={draft.documentDate} onChange={(e) => setDraft({ ...draft, documentDate: e.target.value })} /></div><div className="field"><label>{dict.vendor}</label><input value={draft.vendor} onChange={(e) => setDraft({ ...draft, vendor: e.target.value })} /></div><div className="field"><label>{dict.amount}</label><input type="number" min="0" step="0.01" value={draft.amount} onChange={(e) => setDraft({ ...draft, amount: e.target.value })} /></div><div className="field span2"><label>{dict.assets}</label><div className="tag-editor">{draft.assetIds.length === 0 && <span>{dict.noAssets}</span>}{draft.assetIds.map((id) => { const asset = assets.find((a) => a.id === id); return <span className="tag" key={id}>{asset ? `${asset.nameAr} (${asset.assetNumber})` : id}<button type="button" onClick={() => setDraft({ ...draft, assetIds: draft.assetIds.filter((x) => x !== id) })}>×</button></span>; })}</div><select value="" onChange={(e) => e.target.value && setDraft({ ...draft, assetIds: [...draft.assetIds, e.target.value] })}><option value="">{dict.addAsset}</option>{assets.filter((a) => !draft.assetIds.includes(a.id)).map((a) => <option key={a.id} value={a.id}>{a.assetNumber} — {a.nameAr}</option>)}</select></div><div className="field span2"><label>{dict.notes}</label><textarea value={draft.notes} onChange={(e) => setDraft({ ...draft, notes: e.target.value })} /></div></form>{error && <p role="alert" style={{ color: "var(--seal)" }}>{error}</p>}{editing && <AttachmentUploader ownerType="ASSET_ACQUISITION" ownerId={editing.id} dict={attachmentsDict} canManage={canManage} onAction={() => setToast(commonDict.actionSuccess)} />}</div><div className="modal-foot">{editing && <><button className="btn btn-seal btn-sm" type="button" onClick={() => void remove()}>{dict.delete}</button><button className="btn btn-outline btn-sm" type="button" onClick={() => void print(editing)}>{commonDict.print}</button></>}<div style={{ flex: 1 }} /><button className="btn btn-outline btn-sm" type="button" onClick={() => setEditing(undefined)}>{commonDict.cancel}</button><button className="btn btn-primary btn-sm" form="acquisition-form" type="submit" disabled={saving}>{saving && <span className="spinner" />}{dict.save}</button></div></div></div>}
