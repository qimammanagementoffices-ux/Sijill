...
- **Post-restore process restart:** confirmed live that a successful restore (schema reset + `pg_restore`) leaves the still-running app instance's Hikari pool and Hibernate metadata stale against the new schema — every DB-backed endpoint 500'd until the service was manually restarted. `restore()` now schedules `System.exit(0)` a few seconds after a successful restore (or successful self-heal rollback), relying on Render's documented behavior of restarting a web service whose process exits. Any other in-flight request at that moment gets dropped — accepted, since a restore already invalidates every other active session's view of the database regardless.
