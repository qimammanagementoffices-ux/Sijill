...
- **A successful restore restarts `sijill-api`.** The schema-reset-and-restore process leaves the still-running app instance's DB connection pool stale, so the backend deliberately exits a few seconds after a successful restore; Render restarts the container automatically. Expect a brief (usually under a minute) window where the API is unreachable right after a restore completes — this is expected, not a second incident. If `/actuator/health` doesn't come back within a couple of minutes, check the Render dashboard directly.
- If `pg_restore` itself fails partway (corrupt/incompatible dump, connection drop mid-restore), the backend automatically attempts to roll back to the pre-restore snapshot it just took, rather than leave the database empty. The error response says which happened: rolled back successfully (retry later, or investigate the original dump), or — rare, and serious — rollback also failed, naming the pre-restore snapshot id to restore from manually via `pg_restore` on a scratch instance immediately.
