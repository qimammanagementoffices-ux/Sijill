...
public class NeedRequestController {
    ...
    @PreAuthorize("hasAuthority('emp.manage')")
    public ResponseEntity<Void> restore(@PathVariable UUID id, @AuthenticationPrincipal Employee actor) {
        // The card immediately reloads the archive after this succeeds, so
        // serialising the full aggregate (lines + complete action timeline)
        // is wasted work and can make an old, busy request look stuck.
        needRequestService.restore(id, actor);
